...
def calculer_prediction_old(df_input, df_corr, colonne_valeurs, cle_geo='codeMaille10Km',cle_ID='cdRef'):
    """
...
def calculer_prediction(df_input, df_corr, colonne_valeurs, cle_geo='codeMaille10Km', cle_ID='cdRef'):
    ...
    Cette fonction multiplie les valeurs d'une matrice de corrélation filtrée par une colonne spécifique du DataFrame df_input.
    Le résultat est stocké dans une nouvelle colonne nommée `colonne_valeurs_predit`.
    Affiche aussi une estimation du temps total et l'heure de fin.
    """
    df_input = df_input.sort_values(by=cle_ID, ascending=True)
    ...
    total_maille = len(mailles)  # Nombre total de mailles à traiter
    print(f"Nombre total de mailles: {total_maille}")
    
    start_time = time.time()  # Temps de début
    current_maille = 0        # Compteur d'avancement pour les mailles
    last_percent = -1         # Suivi du dernier pourcentage affiché
    first_percent_time = None # Temps pour le premier pourcentage
    ...
    for maille in mailles:
        ...
        try:
            ...
            # Effectuer la multiplication matricielle
            df_input.loc[df_input[cle_geo] == maille, colonne_resultat] = np.dot(
                df_corr_filtered.values, df_maille[colonne_valeurs].values
            )
        ...
        if percent==1 and percent > last_percent:
            ...
            if first_percent_time is None:
                ...
                heure_fin_estimee = datetime.now() + timedelta(seconds=temps_total_estime)
            else:
                elapsed = now - start_time
                temps_total_estime = elapsed / (percent / 100)
                heure_fin_estimee = datetime.now() + timedelta(seconds=(temps_total_estime - elapsed))
        
            print(f"Progress: {percent}% | Temps total estimé : {temps_total_estime:.1f}s | Heure de fin estimée : {heure_fin_estimee.strftime('%H:%M:%S')}")
            last_percent = percent
