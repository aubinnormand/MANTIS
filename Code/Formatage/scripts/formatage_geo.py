...
def build_country_grid_metric(gdf_country, grid_size_km, cle_geo, crs_metric="EPSG:6933"):
    country_m = gdf_country.to_crs(crs_metric)

    geom = country_m.geometry.unary_union
    minx, miny, maxx, maxy = geom.bounds

    size_m = grid_size_km * 1000

    xs = np.arange(minx, maxx + size_m, size_m)
    ys = np.arange(miny, maxy + size_m, size_m)

    rects, cle_list, rx, ry = [], [], [], []

    for i, x in enumerate(xs[:-1]):
        for j, y in enumerate(ys[:-1]):
            rects.append(box(x, y, x + size_m, y + size_m))
            cle_list.append(f"{i}_{j}")
            rx.append(i)
            ry.append(j)

    grid = gpd.GeoDataFrame(
        {cle_geo: cle_list, "rank_x": rx, "rank_y": ry, "geometry": rects},
        crs=crs_metric
    )

    grid = gpd.overlay(grid, country_m, how="intersection")
...
def generate_country_grid(
    ...
    #grid = build_country_grid_metric(country,grid_size_km=grid_size_km,cle_geo=cle_geo,crs_metric="grid"    )
