...
def calculer_prediction(df_input, df_corr, colonne_valeurs,
                        cle_geo='codeMaille10Km', cle_ID='cdRef'):

    df_input = df_input.copy()

    # 🔒 Forcer les types UNE FOIS
    df_input[cle_ID] = df_input[cle_ID].astype(str)
    df_input[cle_geo] = df_input[cle_geo].astype(str)

    df_corr = df_corr.copy()
    df_corr.index = df_corr.index.astype(str)
    df_corr.columns = df_corr.columns.astype(str)

    df_input = df_input.sort_values(by=cle_ID)

    colonne_resultat = f"{colonne_valeurs}_predit"
    df_input[colonne_resultat] = np.nan

    mailles = df_input[cle_geo].unique()
    total_maille = len(mailles)
    print(f"Nombre total de mailles: {total_maille}")

    start_time = time.time()
    current_maille = 0
    last_percent = -1
    first_percent_time = None

    for maille in mailles:

        df_maille = df_input[df_input[cle_geo] == maille]

        # Espèces présentes dans la maille
        species_maille = df_maille[cle_ID].unique()

        # Espèces communes avec la matrice
        common_species = np.intersect1d(species_maille, df_corr.index)

        # 🚨 Aucune espèce commune → pas de prédiction
        ...
        valeurs = (
            df_maille
            .set_index(cle_ID)
            .loc[common_species, colonne_valeurs]
        )

        # Produit matriciel
        prediction = df_corr_filtered.values @ valeurs.values

        df_input.loc[
            (df_input[cle_geo] == maille) &
            (df_input[cle_ID].isin(common_species)),
            colonne_resultat
        ] = prediction
        ...
        if percent == 1 and percent > last_percent:
            now = time.time()
            if first_percent_time is None:
                first_percent_time = now
                temps_total_estime = (now - start_time) / (percent / 100)
            else:
                elapsed = now - start_time
                temps_total_estime = elapsed / (percent / 100)

            heure_fin_estimee = datetime.now() + timedelta(
                seconds=temps_total_estime
            )

            print(
                f"Progress: {percent}% | "
                f"Temps total estimé : {temps_total_estime:.1f}s | "
                f"Heure de fin estimée : {heure_fin_estimee.strftime('%H:%M:%S')}"
            )
