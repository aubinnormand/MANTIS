...
def afficher_fond_carte(nom, dictionnaire_cartes, source_fond='OpenStreetMap', zoom_manuel=None):
    if nom not in dictionnaire_cartes:
        print(f"Erreur : '{nom}' non trouvé dans la liste des cartes.")
        return
    
    params = dictionnaire_cartes[nom]
    zoom = zoom_manuel if zoom_manuel is not None else params["zoom"]
    
    fig, ax1 = configurer_carte(
        source_fond,
        params["center_x"], params["center_y"], params["height"],
        zoom=zoom, fig_size=(params["size_x"], params["size_y"])
    )
...
def ajouter_couche_continue(fig, ax1, df_inpn,df_geo,col_valeur,cle_geo,quantile_inf=0,quantile_sup=1, borne_min=None,borne_max=None,cmap_choice='viridis',val_alpha=0.75,missing_color=None,colorbar_choice=True,log_values=False,sig=3):
    ...
    val_min = round_to_sig(gdf[col_valeur].quantile(quantile_inf, interpolation='linear'), direction='down',sig=sig)
    val_max = round_to_sig(gdf[col_valeur].quantile(quantile_sup, interpolation='linear'), direction='up',sig=sig)
...
def afficher_carte_interactive(carte_maille: gpd.GeoDataFrame,cle_geo='codeMaille10Km') -> folium.Map:
    ...
    centroid = carte_maille.geometry.unary_union.centroid
